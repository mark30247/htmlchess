...
var ATTR_ORIENTATION	= "orientation";
...
var MERIDA_COORD_COLS_REV 			= 'hgfedcba';

var MERIDA_COORD_ROWS 				= '8<br/>7<br/>6<br/>5<br/>4<br/>3<br/>2<br/>1<br/>';
var MERIDA_COORD_ROWS_REV 			= '1<br/>2<br/>3<br/>4<br/>5<br/>6<br/>7<br/>8<br/>';
...
var MERIDA_FRAME_COLS_REV 			= 'hgfedcba';

var MERIDA_FRAME_ROWS 				= '12345678';
var MERIDA_FRAME_ROWS_REV 			= '87654321';
...
/* My Board object */
function ChessBoard(_fenCode,_attrBorder,_colorMode,_attrStyle,_attrClass, _attrOrientation)
{
	...
	this.attrOrientation = _attrOrientation
	...
	this.parseFen = function()
	{
		...
		var whooseMove =this.fenCode.match(/\s+[b|w]\s*$/);
		if (whooseMove != null)
		{
			this.fenCode=this.fenCode.replace(/\s+[b|w]\s*$/,"");	
			whooseMove=whooseMove[0].trim();
			this.attrOrientation="b"==whooseMove ? "black" : "white";
		}
	...
	this.genColorHTML = function()
	{
	    ...
	    var makeReverse = this.attrOrientation=="black";
		var merida_coord_cols = makeReverse ? MERIDA_COORD_COLS_REV : MERIDA_COORD_COLS;	
		var merida_coord_rows = makeReverse ? MERIDA_COORD_ROWS_REV : MERIDA_COORD_ROWS;
		var generatedHTML="";
	    ...
	    for(row=makeReverse ? this.rowCount-1: 0; makeReverse ? row >= 0 : row<this.rowCount; makeReverse ? row-- : row++)
	    {
			...
			if(makeReverse)
			{
				...
				inputRow=inputRow.replace(/\(/g,"{").replace(/\)/g,"}");
				inputRow=inputRow.replace(/{/g,")").replace(/}/g,"(");
			}
		
	        //for(col=0;col<inputRow.length;col++)
	        for(col=makeReverse ? inputRow.length-1: 0;makeReverse ? col >= 0: col<inputRow.length; makeReverse ? col-- :col++)
	    	{
...
function parseChess()
{
	...
	for (idx=0; idx<chessElt.length;idx++)
	{
		...
		var attrOrientation	= (chessElt[idx].getAttribute(ATTR_ORIENTATION) != null) ? chessElt[idx].getAttribute(ATTR_ORIENTATION) : "";
		...
		var board = new ChessBoard(boardFenCode, attrBorder, attrMode, attrStyle, attrClass, attrOrientation);
